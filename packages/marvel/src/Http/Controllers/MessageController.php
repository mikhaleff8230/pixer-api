...
class MessageController extends CoreController
{
    ...
    public function fetchMessages(Request $request)
    {
        ...
        try {
            ...
            return $this->repository->where('conversation_id', $conversation_id)
                ->with(['conversation.shop', 'conversation.user.profile', 'chatAttachments'])
                ->orderBy('id', 'DESC');
        } catch (\Exception $e) {
            throw new MarvelException(NOT_AUTHORIZED);
        }
    ...
    public function markAsRead(Request $request, $conversation_id)
    {
        ...
        abort_unless(
            $user->shop_id === $conversation->shop_id || 
        ...
        \Marvel\Database\Models\Message::where('conversation_id', $conversation_id)
            ->where('user_id', '!=', $user->id)
            ->whereNull('read_at')
            ->update(['read_at' => now()]);
